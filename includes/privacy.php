...
function replace_gravatar_with_own_images($url, $id_or_email, $args) {
...
add_filter('get_avatar_url', 'replace_gravatar_with_own_images', 10, 3);
...
##################
*/
function remove_comment_cookie_field($fields) {	
	unset($fields['cookies']);		
	return $fields;
}
add_filter('comment_form_default_fields', 'remove_comment_cookie_field');
...
##################
*/
function replace_youtube_nocookie_oembed( $html ) {
	return str_replace('youtube.com', 'youtube-nocookie.com', $html);
}
add_filter('embed_oembed_html', 'replace_youtube_nocookie_oembed');
